...
function makeFakeContext(config: Record<string, unknown>, capturedDiagnostics: CapturedDiagnostic[]): Context {
    const fake: Partial<Context> = {
        ...
        mapUserPath: async (userPath: string) => {
            // BufferProvider strips the leading `file://` before calling mapUserPath.
            // On Windows, this can yield `/X:/path/...` (or occasionally `//X:/path/...`);
            // normalize to `X:/path/...`.
            const withoutLeadingSlashes = userPath.replace(/^\/+([A-Za-z]:\/+)/, '$1');
            // file URLs may contain %-escapes (spaces, unicode); decode to a real path.
...
suite('#iVertex', () => {
    test('WebGL2 mode: loads vertex shader file and strips leading #version', async () => {
        ...
        const vertexUrl = pathToFileURL(vertexFile).toString();
        const code = `// Inlined unit-test shader: uses #iVertex\n#iVertex '${vertexUrl}'\n\nvoid mainImage(out vec4 fragColor, in vec2 fragCoord)\n{\n    fragColor = vec4(1.0);\n}\n`;

        const diags: CapturedDiagnostic[] = [];
        const provider = new BufferProvider(makeFakeContext({ webglVersion: 'WebGL2' }, diags));
        ...
        assert.strictEqual(/#iVertex\s*\x22/m.test(buffer.Code), false, 'Expected #iVertex directive line to be removed from fragment code');
        assert.ok(
            diags.every((d) => !d.message.includes('Custom vertex shaders (#iVertex) require shader-toy.webglVersion')),
            'Expected no WebGL2-gating diagnostics for valid #iVertex in WebGL2 mode'
        );
        assert.ok(
            diags.every((d) => !d.message.toLowerCase().includes('could not open vertex shader file')),
    ...
    test('Default mode: #iVertex is rejected and does not set VertexCode', async () => {
        ...
        const vertexUrl = pathToFileURL(vertexFile).toString();
        const code = `#iVertex '${vertexUrl}'\nvoid mainImage(out vec4 fragColor, in vec2 fragCoord) { fragColor = vec4(0.0); }\n`;

        const diags: CapturedDiagnostic[] = [];
        const provider = new BufferProvider(makeFakeContext({ webglVersion: 'Default' }, diags));
        const buffers: Types.BufferDefinition[] = [];
        const includes: Types.IncludeDefinition[] = [];
        ...
        assert.ok(
            diags.some((d) => d.message.includes('Custom vertex shaders (#iVertex) require shader-toy.webglVersion')),
