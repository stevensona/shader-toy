...
export class AdvanceTimeExtension implements WebviewExtension {
    public generateContent(): string {
        return `\
// When pauseWholeRender is enabled, we still sometimes render a single frame while paused
// (e.g. to reflect sequencer-driven uniform changes). In that case, we must NOT advance time.
if (paused == false) {
